...
function render() {
  ...
  list.querySelectorAll('input[data-document-id]').forEach((checkbox) => checkbox.addEventListener('change', (event) => {
    sources.get(event.target.dataset.documentId).selected = event.target.checked; conversationId = null; conversationScope = ''; updateReadiness();
  }));
  updateReadiness();
}
...
document.querySelector('#chat-form').addEventListener('submit', async (event) => {
  ...
  const currentScope = scopeKey(documentIds);
  if (!conversationId || conversationScope !== currentScope) {
    ...
    conversationScope = currentScope;
  }
  ...
  if (!response.ok) { const error = await response.json().catch(() => ({})); readiness.textContent = `${error.error?.message || 'The question could not be answered.'} (sources: ${documentIds.length})`; updateReadiness(); return; }
