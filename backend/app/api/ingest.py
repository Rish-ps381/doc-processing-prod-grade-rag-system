...
async def get_ingestion(job_id: str, request: Request) -> dict:
    ...
    return {"job_id": job.id, "document_id": job.document_id, "status": job.status.value, "error": job.error.model_dump() if job.error else None, "created_at": job.created_at, "started_at": job.started_at, "completed_at": job.completed_at}
